...
export function FlashcardsPanel({ subjectFilter }: FlashcardsPanelProps) {
  ...
  function practiceOne(card: Flashcard) {
    setPracticeQueue([card])
  }

  function practiceAll() {
    if (visibleFlashcards.length === 0) return
    setPracticeQueue(visibleFlashcards)
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex justify-end gap-2">
        {visibleFlashcards.length > 0 && (
          <Button type="button" variant="outline" onClick={practiceAll}>
          ...
          {visibleFlashcards.map((card) => {
            ...
            return (
              <Card
                ...
                role="button"
                tabIndex={0}
                onClick={() => practiceOne(card)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' || e.key === ' ') practiceOne(card)
                }}
      ...
      <Dialog
        open={practiceQueue !== null}
        onOpenChange={(open) => !open && setPracticeQueue(null)}
      >
        <DialogContent className="sm:max-w-lg">
          ...
          {practiceQueue && (
            <ReviewSession
              queue={practiceQueue}
              onComplete={() => setPracticeQueue(null)}
            />
          )}
        </DialogContent>
      </Dialog>
